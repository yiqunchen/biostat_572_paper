...
with open('./real_data_allen.pkl', 'rb') as fp:
    real_data_dict = pickle.load(fp)

detected_fluo = real_data_dict['detected_fluo']
time_fluo = real_data_dict['time_fluo'] 
# time_spike = real_data_dict['time_spike'] 
detected_fluo = detected_fluo.astype('float')
# we try different cutoffs for sensitivity analysis
L_seq = [0, 0.125, 0.25, 0.375, 0.5]
lambda_choice = np.logspace(-4, 2, num=10)
spike_sigma = 0.9817
#### fix name

test_result = cv_select_lambda(y= detected_fluo,s_vec=None,t_vec=time_fluo,
                   lambda_seq = lambda_choice ,L_min_seq = L_seq,\
                   gamma =spike_sigma)

output_dir = '/home/students/yiqunc/stat572/output/real_data/'


with open(output_dir+'more_lambda_allen_100s_data.pkl', 'wb') as handle:
